...
class PdfViewerConfig {
  ...
  /// 获取下一个章节的回调
  final dynamic Function()? getNextNode;

  /// 获取上一个章节的回调
  final dynamic Function()? getPreviousNode;

  /// 切换到下一章的回调
  final void Function()? moveToNextChapter;

  /// 切换到上一章的回调
  final void Function()? moveToPreviousChapter;

  /// 检查节点是否有效的回调
  final bool Function(dynamic node)? isNodeValid;
...
class _CommonPdfViewerState extends State<CommonPdfViewer> {
  ...
  void initState() {
    ...
    // 监听 PDF URL 变化
    widget.config.pdfUrlStream.listen((url) async {
      if (url != null && mounted) {
        // 切换PDF时，保存当前页码
  ...
  void _onPageChanged(PdfPageChangedDetails details) {
    ...
    if (!widget.config.enableChapterNavigation) return;
  ...
  void _tryNavigateToNextChapter() {
    if (widget.config.getNextNode == null ||
        widget.config.moveToNextChapter == null ||
        widget.config.isNodeValid == null) {
      return;
    }

    final nextNode = widget.config.getNextNode!();
    if (widget.config.isNodeValid!(nextNode)) {
      setState(() => _isChangingPage = true);
      widget.config.moveToNextChapter!();
      if (mounted) {
  ...
  void _tryNavigateToPreviousChapter() {
    if (widget.config.getPreviousNode == null ||
        widget.config.moveToPreviousChapter == null ||
        widget.config.isNodeValid == null) {
      return;
    }

    final previousNode = widget.config.getPreviousNode!();
    if (widget.config.isNodeValid!(previousNode)) {
      setState(() => _isChangingPage = true);
      widget.config.moveToPreviousChapter!();
      if (mounted) {
        setState(() => _isChangingPage = false);
      }
    }
  ...
  Widget _buildPdfContent() {
    return Obx(() {
      final selectedUrl = widget.config.pdfUrlStream.value;
            ...
            Center(
              child: Container(
                ...
                color: widget.backgroundColor,
                  child: Padding(
                    padding: EdgeInsets.symmetric(
                        horizontal: _screenAdapter.getAdaptivePadding(120.0)),
                    child:  SfPdfViewer.file(
